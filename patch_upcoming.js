...
export function UpcomingEpisodeRow({ 
  ...
  return (
    <div className="flex items-center justify-between py-3 pr-4 bg-black transition-all hover:bg-white/5 rounded-md overflow-hidden h-[120px]">
      <Link href={\`/show/\${show.id}\`} className="flex items-center gap-4 flex-1 overflow-hidden cursor-pointer h-full">
        ...
        <img 
          ...
          alt={show.name} 
          className="w-[72px] h-full object-cover rounded-md shadow-md bg-[#1E1E1E]"
        />
        <div className="flex flex-col justify-center overflow-hidden py-1">
          ...
          <div className="flex items-center leading-tight">
            <span className="font-bold text-[15px] text-white">S{String(episode.season).padStart(2, '0')} | E{String(episode.episode).padStart(2, '0')}</span>
          </div>
          <span className="text-[13px] text-gray-300 line-clamp-1 mt-0.5">{episode.name || \`Episode \${episode.episode}\`}</span>
...
fs.writeFileSync('src/app/(main)/shows/UpcomingEpisodeRow.tsx', content);
