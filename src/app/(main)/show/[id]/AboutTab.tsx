...
export function AboutTab({ show }: { show: TMDBShowDetails }) {
  // Format subtitle
    ...
    <div className="flex flex-col gap-6 p-4">
      ...
      <section className="flex flex-col gap-2 border-b border-[#2A2A2A] pb-6">
        ...
        {trailer && (
          <div className="flex items-center gap-4 mt-4 cursor-pointer hover:bg-white/5 p-2 -ml-2 rounded-lg transition-colors">
            <div className="relative w-24 h-14 rounded-md overflow-hidden bg-gray-800 flex-shrink-0">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <PlayCircle className="text-white drop-shadow-lg" size={28} strokeWidth={1.5} />
              </div>
            </div>
        ...
        <section className="flex flex-col gap-3 border-b border-[#2A2A2A] pb-6">
          ...
          <div className="flex overflow-x-auto gap-3 pb-2 -mx-4 px-4 snap-x hide-scrollbar">
            {show.credits.cast.slice(0, 15).map(person => (
              <div key={person.id} className="relative flex flex-col w-28 h-40 rounded-lg overflow-hidden shrink-0 snap-start bg-gray-800">
                {person.profile_path ? (
                  // eslint-disable-next-line @next/next/no-img-element
                ...
                <div className="absolute bottom-0 left-0 right-0 p-2 flex flex-col">
                  ...
                  <span className="text-[9px] font-bold text-gray-400 uppercase line-clamp-1">{person.character}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="flex flex-col gap-4 pb-6">
        <div className="flex justify-between items-end">
          <h2 className="text-lg font-bold text-white">Community ratings</h2>
          <span className="text-sm text-gray-400">Season 1</span>
        </div>
        
        {/* Mock Rating Graph */}
        <div className="h-24 border-b border-gray-700 relative flex items-end">
           <div className="absolute inset-0 flex flex-col justify-between text-[10px] text-gray-500 pb-1 pr-1">
